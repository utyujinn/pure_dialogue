...
一人のときは AI が相手をし、他のユーザーが接続すると自動で人間同士の対話に切り替わる。

---

## 機能

- **AI 対話モード**: 一人のときはマイクに話しかけるだけで AI キャラクターが応答する
- **人間対話モード**: 2 人以上が接続すると自動で切り替わる。発話は STT → TTS を経由して相手に届くため、声質・年齢・性別の差が吸収され全員が同じアバター音声で話せる
- **リアルタイム通話**: 📞 ボタンでリクエストを送り、相手が承認すると生 PCM をリアルタイム転送する低遅延通話モードに入る
- **リップシンク・瞬き**: 音声 RMS に基づいて口の形態素を制御。ランダムな瞬きアニメーション
- **ペルソナ切り替え**: 6 種類のキャラクター設定（お姉ちゃん・知的・幼児・ツンデレ・先生・大人なお姉さん）
- **親密度システム**: 会話のたびに LLM が +1 / 0 / -1 を判定。Cookie に保存され次回も引き継がれる。親密度に応じてキャラクターの態度が変化する（AI モード時のみ）
...
[FastAPI]                             [FastAPI]
  STT (Whisper)                         STT (Whisper)
      ...
      ▼                                      ▼
  TTS (VoiceVox / IrodoriTTS)          TTS (VoiceVox / IrodoriTTS)
      │ WAV（統一音声）                       │ WAV（統一音声）
      ▼                                      ▼
[ブラウザ B に送信]                    [ブラウザ A に送信]

  ※ LLM は呼ばない。声質は TTS によって統一される。
```

### リアルタイム通話モード（通話ボタンで開始）

```
[ブラウザ A]  ScriptProcessorNode → PCM パケット → [FastAPI] → [ブラウザ B]
[ブラウザ B]  ScriptProcessorNode → PCM パケット → [FastAPI] → [ブラウザ A]

  STT/TTS をバイパスして生 PCM を即時転送。遅延最小。
  パケット形式: [4 bytes: senderSampleRate (Uint32LE)][Float32[] PCM]
```
...
| フォーマット | 内容 |
|---|---|
| `bytes` | WebM/Opus 音声チャンク（AI・人間対話モード）または PCM パケット（通話モード） |
...
| `{"type":"set_username","username":"..."}` | ユーザー名を設定 |
| `{"type":"call_request"}` | 通話リクエストを送信 |
| `{"type":"call_accept"}` | 通話リクエストを承認 |
| `{"type":"call_decline"}` | 通話リクエストを拒否 |
| `{"type":"call_end"}` | 通話を終了 |
| `{"type":"flush_audio_buf"}` | サーバー側の音声バッファを破棄（通話終了後に送信） |

#### サーバー → ブラウザ

| フォーマット | 内容 |
|---|---|
| `bytes` | WAV（PCM 16bit）音声。AI 応答または相手の TTS 音声 |
| `bytes` | PCM パケット（通話モード時のみ。相手からの生音声） |
| `{"type":"transcript","text":"...","from":"..."}` | Whisper の認識テキスト |
| `{"type":"llm_text","text":"..."}` | LLM のトークン（リアルタイム字幕用、AI モード時のみ） |
| `{"type":"peer_text","text":"...","from":"..."}` | 相手の発話テキスト（人間対話モード時） |
| `{"type":"done"}` | 全処理完了。ブラウザ側はこれを受けてマイクを再起動する |
| `{"type":"affinity","delta":1}` | 今回の会話の親密度変化量（+1 / 0 / -1、AI モード時のみ） |
| `{"type":"mode","mode":"ai"\|"human","users":N}` | 接続人数に応じたモード変化通知 |
| `{"type":"call_request","from":"..."}` | 着信通知 |
| `{"type":"call_start"}` | 通話開始（両者に送信） |
| `{"type":"call_end"}` | 通話終了（両者に送信） |
| `{"type":"call_declined"}` | 通話拒否通知（発信者に送信） |
...
2. 許可するとすぐに音声認識が始まる。話しかけるだけで自動検知・AI が応答する
3. 別のユーザーが同じ URL にアクセスすると自動で人間対話モードに切り替わる。お互いの声は STT → TTS を経由して統一アバター音声で届く
4. 📞 ボタンで通話リクエストを送ると、相手が承認した場合に低遅延のリアルタイム通話モードに入る
5. ⚙️ ボタンで設定パネルを開き、ペルソナ・TTS・話者・AI モデル・マイク・ユーザー名を変更できる
6. 画面左の縦型ゲージが親密度を表示（AI モード時のみ更新）
7. 画面をドラッグでカメラ回転、スクロールでズーム
